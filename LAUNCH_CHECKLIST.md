...
- [X] `git clone` del repo + `--import` + sanity check dei test — [SETUP_VPS §6](SETUP_VPS.md#6-deploy-del-codice)
...
- [X] Applicare lo schema con `db/apply.sh`, poi impostare la password di `autochess_auth` — [SETUP_DB §4](SETUP_DB.md#4-applicare-lo-schema) + [§3 (ruolo)](SETUP_DB.md#ruolo-di-connessione-di-postgrest)
- [X] Installare il binario **PostgREST**, `/etc/autochess/postgrest.conf`, unit `autochess-postgrest` — [SETUP_DB §5](SETUP_DB.md#5-postgrest)
- [X] Creare il **client OAuth Google "Desktop app"** su Google Cloud Console — [SETUP_DB §2](SETUP_DB.md#2-google-cloud-console--oauth-client-id)
- [X] Compilare `/etc/autochess/env` (`DB_API_URL`, `GOOGLE_CLIENT_ID/SECRET`, `SESSION_TOKEN_SECRET`, `MATCH_TOKEN_SECRET`, `BACKUP_*`) — [SETUP_DB §6](SETUP_DB.md#6-secret-del-server)
- [X] Installare **Caddy** con il `Caddyfile` (TLS automatico) — [SETUP_VPS §8](SETUP_VPS.md#8-caddy)
- [X] `systemd`: abilitare `autochess-postgrest`, `autochess-master`, `autochess-worker@1` — [SETUP_VPS §9](SETUP_VPS.md#9-systemd--postgrest-master-worker)
...
- [X] Verifica end-to-end: `wscat -c wss://<dominio>/ws/mm` si connette senza errori TLS — [SETUP_VPS §11](SETUP_VPS.md#11-verifica-end-to-end)
